...
use crate::member_node::swim_node::{DefaultMemberNode};
...
impl NodeRequestRouter for DefaultNodeRequestRouter {
    ...
    fn send(&mut self, from: u16, to: u16) {
        ...
        let from_node_details = self.routes.get(&from).unwrap().lock().unwrap().details().serialize();
    ...
    fn shut_down(&self) {
        for node in self.routes.values() {
            self.connection_factory.lock().unwrap().send_to(node.lock().unwrap().details().host(), Message::Shutdown());
        }
    }
}
